...
//TODO: Change this to a Mat pointer
Mat * ImageProcessing::performMeanShift(const Mat &input, bool writeFiles) {
    Mat originalImage, labImg, res, gray, segColor;
    Mat *openClose;
...
void ImageProcessing::takeImage(Mat &img) {
    VideoCapture capture(0); // camera object FIXME make this member variable?
    capture >> img; // Hey Joe! This is the original image.
    capture.release();
}

void ImageProcessing::takeAverageImage(Mat &img, size_t frames) {
