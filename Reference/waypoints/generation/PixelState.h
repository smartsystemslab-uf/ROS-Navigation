  ...
  // calculates the heuristic value of this pixel (future distance to goal)
	...
	void drawCircles(const int gridSize, Mat &map) {
		if (gridSize < 8) {
			...
			Mat tmp = imread("waypointsz.jpg");
			Vec3b color = tmp.at<Vec3b>(Point(x, y));
			if (color == Vec3b(255,255,255))
				color = Vec3b(25,10,10);
			if (color[0] < 230)
				color[0] += 30;
			cv::rectangle(map, Point(x-4, y-4), Point(x+4, y+4), color, CV_FILLED, LINE_8, 0);
		}
		else {
			// Mat tmp = imread("waypointsz.jpg");
			// Vec3b color = tmp.at<Vec3b>(Point(x, y));
			...
			// if (color[0] < 230)
			// 	color[0] += 30;
			// cv::rectangle(map, Point(x-6, y-6), Point(x+6, y+6), color, CV_FILLED, LINE_8, 0);
    	map.at<Vec3b>(Point(x, y)) = Vec3b(255,20,20);
